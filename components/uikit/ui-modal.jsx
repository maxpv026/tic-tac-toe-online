     ...
     <div
       ...
       className={clsx(
         "bg-white rounded-lg min-h-[320px] mx-auto relative flex flex-col",
         {
           ...
           "w-full mx-5": width === "full",
         }
       )}
     >
       <button
         onClick={onClose}
         className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-white/40 bg-white/10 transition-colors absolute top-0 left-[calc(100%+12px)]"
       >
...
UiModal.Header = function UiModalHeader({ children, className }) {
 return <div className={clsx(className, "pt-6 px-6 pb-4 text-2xl")}>{children}</div>;
};
